...
pub use types::{
    ...
    ScoreSubmissionWithProof, ScoreTrend, UpgradeProposal, SnapshotRecord,
    EffectiveRiskScore, ModelVersionStats,
};
...
impl LedgerLensScoreContract {
    ...
    pub fn query_risk_gate(
        ...
        Self::query_gate_with_confidence(env, wallet, asset_pair, gate_threshold, 0)
    }
    ...
    //
    // Returns [`Error::InvalidConfigValue`] when `threshold` is `0` or exceeds
    ...
    // # Errors
    // - [`Error::NotInitialized`] if the contract has no admin yet.
    // - [`Error::InvalidConfigValue`] if `delay_secs` is outside the bounds.
    ...
    //
    // The value is rejected with [`Error::InvalidConfigValue`] when it
    ...
    //
    // Returns [`Error::InvalidConfigValue`] when `threshold` is `0` or
    ...
    fn write_score_with_rate_limit(
        ...
        // Capture the previous score before overwriting, so score-delta and
        // historical-max logic have an accurate baseline.  peek_score is used
        // to avoid extending the entry's TTL as a side effect of this read.
        let previous_score: Option<u32> =
            storage::peek_score(env, wallet, asset_pair).map(|s| s.score);

        // Enforce the configurable score-floor policy for high-risk wallets
        // before committing the rate-limit timestamp — a rejected submission
        // must not consume the cooldown window.
        if Self::score_floor_blocks(env, wallet, asset_pair, risk_score.score) {
            return Err(Error::BelowScoreFloor);
        }

        // Commit the rate-limit timestamp only after all pre-checks pass.
        ...
        // Emit score-delta event (mirrors the batch path).
        Self::emit_score_delta(env, wallet, asset_pair, previous_score, risk_score.score);

        // Emit score-jump anomaly event (mirrors the batch path).
        Self::emit_score_jump_anomaly(
            env,
            wallet,
            asset_pair,
    ...
    // ── Wallet Relationship Graph / Contagion ─────────────────────────────────

    // Bidirectionally links two wallets as trading counterparties for an asset pair.
    //
    // Service-authorized. Restricts links count to `MAX_COUNTERPARTY_LINKS_PER_WALLET`.
    ...
    // ── Issue-specific Query Functions ──────────────────────────────────────

    // Returns the total count of registered admin signers.
    // When the multi-sig admin set is populated it is authoritative;
    // when only the single-admin slot is occupied the count is 1.
    pub fn get_admin_signer_count(env: Env) -> u32 {
        let admin_set = storage::get_admin_set(&env);
        if !admin_set.is_empty() {
            admin_set.len()
        } else if storage::has_admin(&env) {
            1
        } else {
            0
        }
    ...
    // Returns the age (in seconds) of the last score submission for a wallet/pair.
    ...
    // Returns the active timed embargo expiry timestamp (in seconds) for a wallet,
    // or None if there is no active timed embargo.
    pub fn get_embargo_expiry(env: Env, wallet: Address) -> Option<u64> {
        let key = crate::types::DataKey::ScoreEmbargo(wallet);
        let expiry: Option<crate::types::EmbargoExpiry> = env.storage().temporary().get(&key);
        match expiry {
            Some(crate::types::EmbargoExpiry::Until(ts)) => {
