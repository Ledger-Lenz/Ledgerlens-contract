...
use crate::types::{
    AggregateRiskScore, DataKey, EmbargoExpiry, HistogramData, ModelVersionStats, RiskScore,
    ScoreFloorPolicy, ScoreTrend, UpgradeProposal,
};
...
pub fn get_signer_ttl(env: &Env) -> u64 {
    let result: Option<(u64, u64)> = env.storage().instance().get(&DataKey::SignerTtl);
    result.map(|(ttl, _)| ttl).unwrap_or(0)
}

pub fn set_signer_ttl(env: &Env, ttl: u64) {
    let grace = get_signer_grace_period(env);
    env.storage().instance().set(&DataKey::SignerTtl, &(ttl, grace));
}

pub fn get_signer_grace_period(env: &Env) -> u64 {
    let result: Option<(u64, u64)> = env.storage().instance().get(&DataKey::SignerTtl);
    result.map(|(_, grace)| grace).unwrap_or(604_800)
}

pub fn set_signer_grace_period(env: &Env, grace: u64) {
    let ttl = get_signer_ttl(env);
    env.storage().instance().set(&DataKey::SignerTtl, &(ttl, grace));
}

pub fn get_signer_activation_time(env: &Env, signer: &Address) -> Option<u64> {
    env.storage().instance().get(&DataKey::SignerActivationTime(signer.clone()))
}

pub fn set_signer_activation_time(env: &Env, signer: &Address, timestamp: u64) {
    env.storage().instance().set(&DataKey::SignerActivationTime(signer.clone()), &timestamp);
}

pub fn clear_signer_activation_time(env: &Env, signer: &Address) {
    env.storage().instance().remove(&DataKey::SignerActivationTime(signer.clone()));
}

// ── Score histogram (Issue #81) ──────────────────────────────────────────────

fn score_to_bucket(score: u32) -> u32 {
    core::cmp::min(score / 10, 9)
}

fn load_histogram(env: &Env) -> HistogramData {
    env.storage()
        .instance()
        .get(&DataKey::ScoreHistogramData)
        .unwrap_or_else(|| {
            let mut buckets: soroban_sdk::Vec<u32> = soroban_sdk::Vec::new(env);
            for _ in 0..10 {
                buckets.push_back(0);
            }
            HistogramData { buckets, total: 0 }
        })
}

fn save_histogram(env: &Env, data: &HistogramData) {
    env.storage().instance().set(&DataKey::ScoreHistogramData, data);
}

pub fn get_histogram_bucket(env: &Env, bucket: u32) -> u32 {
    let data = load_histogram(env);
    data.buckets.get(bucket).unwrap_or(0)
}

pub fn get_histogram_total(env: &Env) -> u32 {
    load_histogram(env).total
}

pub fn load_full_histogram(env: &Env) -> HistogramData {
    load_histogram(env)
}

pub fn increment_histogram(env: &Env, score: u32) {
    let mut data = load_histogram(env);
    let bucket = score_to_bucket(score);
    let count = data.buckets.get(bucket).unwrap_or(0);
    data.buckets.set(bucket, count + 1);
    data.total = data.total.saturating_add(1);
    save_histogram(env, &data);
}

pub fn decrement_histogram(env: &Env, score: u32) {
    let mut data = load_histogram(env);
    let bucket = score_to_bucket(score);
    let count = data.buckets.get(bucket).unwrap_or(0);
    if count > 0 {
    ...
    if data.total > 0 {
        data.total = data.total.saturating_sub(1);
    }
    save_histogram(env, &data);
}
